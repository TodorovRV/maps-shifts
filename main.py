...
def get_core_img_mask(uvfits, mapsize_clean, beam_fractions, path_to_default_script, use_elliptical=False,
    ...
    with TemporaryDirectory() as working_dir:
        ...
        core = detect_core(uvfits, beam_fractions, bmaj, mapsize_clean, freq_hz, path_to_default_script,
        ...
        mask = create_beamlike_mask_from_core(core, mapsize, beam, scale=1.8)
        #if base_dir is not None:
        #    os.remove(os.path.join(base_dir, "difmap.log"))
...
def detect_core(uvfits, beam_fractions, bmaj, mapsize_clean, freq_hz, default_script,
                use_brightest_pixel_as_initial_guess, use_elliptical, working_dir):
...
def registrate_images(image1, image2, mask1, mask2, fit_gaussian=True, n=9, max_shift=500):
    ...
    if fit_gaussian:
        ...
        if abs(result[0]) > max_shift or abs(result[1]) > max_shift:
            print('Fit unsuccessful, returning max position!')
